  ...
  return (
    <div className="bg-white min-h-screen pt-24 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
          <Link to="/dashboard" className="inline-flex items-center text-sm text-slate-500 hover:text-slate-900 transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mb-16">
        ...
        <div className="flex justify-center items-center mt-10 gap-4">
            <span className={`text-sm font-bold ${!annual ? 'text-slate-900' : 'text-slate-500'} transition-colors`}>Monthly</span>
            <button 
                onClick={() => setAnnual(!annual)}
                className="w-16 h-8 bg-slate-900 rounded-full relative transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-900"
            >
            ...
            <span className={`text-sm font-bold ${annual ? 'text-slate-900' : 'text-slate-500'} transition-colors flex items-center gap-2`}>
                Yearly <span className="text-green-700 text-[10px] font-extrabold uppercase bg-green-100 px-2 py-0.5 rounded-full tracking-wider">-20%</span>
            </span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-32">
        <div className="grid lg:grid-cols-3 gap-8 items-start">
            ...
            {/* Free Tier */}
            <div className="p-8 rounded-3xl border border-gray-100 bg-white shadow-sm hover:shadow-xl transition-all duration-300 relative group">
                <h3 className="text-xl font-bold text-slate-900 mb-2">Starter</h3>
                <p className="text-slate-500 text-sm mb-8 leading-relaxed">Perfect for exploring the ecosystem and learning basics.</p>
                <div className="mb-8">
                ...
                <Link to="/signup" className="block w-full py-4 px-6 bg-slate-50 text-slate-900 font-bold text-center rounded-xl hover:bg-slate-100 transition-colors border border-gray-200">
                    Current Plan
                </Link>
            ...
            <div className="p-8 rounded-3xl bg-slate-900 text-white shadow-2xl relative overflow-hidden transform lg:-translate-y-6 lg:scale-105 z-10 border border-slate-800">
                 {/* Decorative Blobs */}
            ...
            {/* Enterprise Tier */}
            <div className="p-8 rounded-3xl border border-gray-100 bg-white shadow-sm hover:shadow-xl transition-all duration-300 relative">
                <h3 className="text-xl font-bold text-slate-900 mb-2">Enterprise</h3>
                <p className="text-slate-500 text-sm mb-8 leading-relaxed">For large teams and organizations requiring custom solutions.</p>
                <div className="mb-8 flex items-center h-[48px]">
                    <span className="text-4xl font-bold text-slate-900 tracking-tight">Custom</span>
                </div>
                <Link to="/contact" className="block w-full py-4 px-6 bg-white border-2 border-slate-100 text-slate-900 font-bold text-center rounded-xl hover:border-slate-900 hover:bg-slate-50 transition-all">
                    Contact Sales
                </Link>
                <div className="mt-10 space-y-5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-32">
          <h2 className="text-3xl font-bold text-slate-900 mb-12 text-center">Compare Plans</h2>
          <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                  <thead>
                      <tr>
                          <th className="py-6 px-4 text-sm font-semibold text-slate-900 border-b border-gray-200 w-1/4">Features</th>
                          <th className="py-6 px-4 text-sm font-bold text-slate-900 border-b border-gray-200 w-1/4 text-center">Starter</th>
                          <th className="py-6 px-4 text-sm font-bold text-blue-600 border-b border-gray-200 w-1/4 text-center bg-blue-50/50 rounded-t-xl">Pro</th>
                          <th className="py-6 px-4 text-sm font-bold text-slate-900 border-b border-gray-200 w-1/4 text-center">Enterprise</th>
                      </tr>
                  </thead>
                  <tbody>
      ...
      {/* Trust Badge */}
      <div className="text-center mt-20 text-slate-400 text-sm font-medium flex items-center justify-center gap-2">
...
const Feature = ({ text, dark = false }: { text: string, dark?: boolean }) => (
    <div className={`flex items-start gap-3 ${dark ? 'text-slate-300' : 'text-slate-600'}`}>
        <div className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center mt-0.5 ${dark ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-50 text-blue-600'}`}>
...
const FaqItem = ({ q, a }: { q: string, a: string }) => (
    <div className="border border-gray-100 rounded-2xl p-6 bg-white hover:border-gray-200 transition-colors shadow-sm">
        ...
        <p className="text-slate-600 text-sm ml-8 leading-relaxed">{a}</p>
    </div>
);
...
const TableRow = ({ feature, starter, pro, ent }: { feature: string, starter: string | boolean, pro: string | boolean, ent: string | boolean }) => {
    const renderCell = (val: string | boolean, isPro = false) => {
        if (val === true) return <Check size={20} className={`mx-auto ${isPro ? 'text-blue-600' : 'text-green-500'}`} />;
        if (val === false) return <Minus size={20} className="mx-auto text-slate-300" />;
        return <span className={`text-sm font-medium ${isPro ? 'text-blue-700' : 'text-slate-600'}`}>{val}</span>;
    };

    return (
        <tr className="hover:bg-gray-50/50 transition-colors">
            <td className="py-4 px-4 border-b border-gray-100 text-sm font-medium text-slate-900">{feature}</td>
            <td className="py-4 px-4 border-b border-gray-100 text-center">{renderCell(starter)}</td>
            <td className="py-4 px-4 border-b border-gray-100 text-center bg-blue-50/20">{renderCell(pro, true)}</td>
            <td className="py-4 px-4 border-b border-gray-100 text-center">{renderCell(ent)}</td>
        </tr>
    );
};
